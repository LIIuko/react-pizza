...
import {clearPizzaCart, minusPizzaCart, plusPizzaCart, removePizzaCart} from "../redux/actions/cart";
...
const Cart = () => {
    ...
    const clearCart = () => {
        if (window.confirm("Вы действительно собираетесь удалить весь товар из корзины?")) {
            dispatch(clearPizzaCart());
        }
    }

    const removeItem = (id) => {
        if (window.confirm("Вы действительно собираетесь удалить товар из корзины?")) {
            dispatch(removePizzaCart(id));
        }
    }

    const plusItem = (id) => {
        dispatch(plusPizzaCart(id));
    }

    const minusItem = (id) => {
        dispatch(minusPizzaCart(id));
    }
        ...
        <div className="cart__container">
            {
                addedPizzas.length
                    ...
                    <>
                        ...
                        <div className="cart__footer">
                            ...
                            <div className="cart__sum">Сумма заказа: <span
                                className="cart__total-price">{totalPrice} ₽</span></div>
                        </div>
                        <div className="cart__footer__buttons">
                            <Link to={'/'}>
                                <button className="cart__exit">
                                    <svg className="cart__exit__logo" xmlns="http://www.w3.org/2000/svg" width="8"
                                         height="14"
                    ...
                    <div className="cart__empty__container">
                        <div className="cart__empty__title__container">
                            <h2 className="cart__title cart__empty__title">Корзина пустая</h2>
                            <img src={smileImg} alt=""/>
                        </div>
                        <div className="cart__empty-desc">Вероятней всего, вы не заказывали ещё пиццу.
                           <br/>Для того, чтобы заказать пиццу, перейди на главную страницу.
                        </div>
                        <img src={emptyCart} alt=""/>
                        <Link to="/">
                            <button className="cart__empty__back">Вернуться назад</button>
                        </Link>
                    </div>
            }
        </div>
